...
// block for IR module
unsigned int busyPins[]= {0, 0, 0, 0, 32, 33, 0, 0, 0, 0};  // array for occupation signal where connected occupation modules
unsigned int itrPins []= {0, 0, 0, 0, 16, 17, 0, 0, 0, 0}; //array for pin where to connect ifr chip data pin used for interrpt 
unsigned int relayPins[]= {19, 19, 0, 0, 0, 0, 0, 0, 0, 0};  // array for Line Halt-Go 
...
void IRAM_ATTR isr_4() {
  idxIR=4;
  valBlock = digitalRead(itrPins[4]); // Read valBlockue from block sensor 
//  Serial.print ("valblock= ");
...
void IRAM_ATTR isr_5() {
  idxIR=5;
  valBlock = digitalRead(itrPins[5]); // Read valBlockue from block sensor 
  if(valBlock==1){    
    blockStat="LD5"; //L=Low D=Deviatoio 5= Number of Turnouts
  }
  if(valBlock==0){    
    blockStat="HD5"; //H=HIGH D=Deviatoio 5= Number of Turnouts
     }
} 
...
//  digitalWrite(BTled, LOW); 
 ...
 Serial.println("The device started, now you can pair it with bluetooth!");
  ...
  attachInterrupt(itrPins[4],isr_4, CHANGE);
  attachInterrupt(itrPins[5],isr_5, CHANGE);
...
void loop()
{
  ...
  if (blockStat!=""){
      Serial.print("blockstate ");
      Serial.println (blockStat);
     // SerialBT.write(blockStat);
      SerialBT.write(blockStat[0]);
      SerialBT.write(blockStat[1]);
      SerialBT.write(blockStat[2]);
      if (blockStat[0] == 'L') {
